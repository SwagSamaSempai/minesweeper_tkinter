...
class Minesweeper:
    def __init__(self, master, infos=None):
        ...
        sys.setrecursionlimit(self.grid.get_height() * self.grid.get_width())
    ...
    def reveal_cell(self, x, y):
        ...
        if self.grid.cells[(x, y)].get_value() == NO_VALUE:
            ...
            for x, y in self.grid.cells[(x, y)].get_surroundings():
                if not self.grid.cells[(x, y)].is_revealed():
                    self.canvas.update()
                    self.reveal_cell(x, y)
        else:
...
def main():
    root = tk.Tk()
    infos = sys.argv[:]
    if "Minesweeper_rec.py" in infos:
        index = infos.index("Minesweeper_rec.py")
        infos = infos[index + 1:]
